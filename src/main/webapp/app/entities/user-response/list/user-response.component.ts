...
import { EntityArrayResponseType, UserResponseService } from '../service/user-response.service';
...
export class UserResponseComponent implements OnInit {
  ...
  trackId = (_index: number, item: IUserResponse): number => this.userResponseService.getUserResponseIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.userResponses = this.refineData(dataFromBody);
  }

  protected refineData(data: IUserResponse[]): IUserResponse[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IUserResponse[] | null): IUserResponse[] {
    return data ?? [];
  }
