...
type RestOf<T extends IContacts | NewContacts> = Omit<T, 'lastDayConnection'> & {
...
export class ContactsService {
  ...
  protected convertDateFromClient<T extends IContacts | NewContacts | PartialUpdateContacts>(contacts: T): RestOf<T> {
    return {
      ...contacts,
      lastDayConnection: contacts.lastDayConnection?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restContacts: RestContacts): IContacts {
    return {
      ...restContacts,
      lastDayConnection: restContacts.lastDayConnection ? dayjs(restContacts.lastDayConnection) : undefined,
    };
  }
